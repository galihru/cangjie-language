...
> 面向全场景智能的新一代编程语言。本文是给入门者的 **README.md** 级别学习资料（中文），涵盖环境搭建、基础语法、常用工具与构建产物说明，并附示例代码与常见问答。
...
**Q1：能否编译成 Windows 的 `.exe`？**  
A：可以。`cjc` 默认即生成可执行文件；亦支持编译为静态/动态库。

**Q2：能否直接生成 Android 的 `.apk`？**  
A：不可以。HarmonyOS NEXT 已去除 Android 兼容层，移动端原生应用以 **HAP** 为主要分发格式。

**Q3：能否用于 AI 开发？**  
A：可以把仓颉作为“**AI 应用开发语言**”。一方面有 AgentDSL 支撑智能体编排；另一方面可经由 FFI 调用 C/Python 等成熟推理库或服务。

**Q4：IDE 支持如何？**  
A：提供 **VS Code 扩展**（语法/编译/调试集成），并有 **cjdb** 调试器；DevEco Studio 可用于鸿蒙工程集成。

---

## 9. 参考与延伸阅读（建议收藏）

- 官方主页与文档：下载、用户手册、语言规范、工具指南、Playground。  
- VS Code 扩展：搜索 “Cangjie” 插件并安装。  
- 包管理与构建：CJPM 使用说明。  
- 社区精选：awesome-cangjie、示例项目与教程博客。
